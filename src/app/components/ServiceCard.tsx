...
export const ServiceCard = ({upperText, bottomText, title, detailsList, priceText, btnText, imgUrl, category, hasButton = true}:ServiceCardProps) => {
  return (
    <div className='service-card min-h-full flex flex-col justify-between max-w-[300px]' id={title.replace(' ', '-').toLowerCase()}>
      ...
      <div>
        <div className={`w-full relative flex flex-col items-center p-4 border-4 border-accent-yellow bg-radial from-white from-60% to-gray-400 rounded-lg text-black`}>
          <div>
            <h3 className='title font-black text-left text-4xl uppercase ml-5'>{upperText}</h3>
        ...
        <div className='my-4 flex flex-col'>
          <h3 className='text-4xl leading-9 font-bold mb-4 '>{title}</h3>
      ...
      <div className='flex flex-col items-start gap-4'>
        <span className='text-accent-yellow font-bold text-xl'>{priceText}</span>
